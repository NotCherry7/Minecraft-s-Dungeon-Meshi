package net.notcherry.dungeonmod.client.overlays;
...
public class SpellWheelOverlay {
    ...
    public final static ResourceLocation TEXTURE = new ResourceLocation(DungeonMod.MOD_ID, "textures/gui/icons.png");
...
//            swsm.makeSelection(wheelSelection);
    ...
    public void onRenderOverlay(TickEvent.RenderTickEvent event) {
        ...
        renderOverlay(screenWidth, screenHeight);
    }

    private void renderOverlay(int screenWidth, int screenHeight) {
        Minecraft minecraft = Minecraft.getInstance();
        int totalSpellsAvailable = 8;
...
//                guiHelper.blit(TEXTURE, (int) locations[i].x - borderWidth, (int) locations[i].y - borderWidth, swsm.getSelectionIndex() == i ? 32 : 0, 106, 32, 32);
                ...
                Cooldown
                 */
//                float f = ClientManaData.getCooldownPercent(spell.getSpell());
//                if (f > 0) {
//                    RenderSystem.enableBlend();
